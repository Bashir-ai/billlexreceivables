...
import { getManagementFeesForUser } from "@/lib/management-fee-helpers"
  ...
  try {
    ...
    if ((session.user.role === UserRole.ADMIN || session.user.role === UserRole.MANAGER) && userId) {
      ...
      return NextResponse.json(
        { error: "Forbidden - External users can only view their own management fees" },
        { status: 403 }
      )
    }
    ...
    const fees = await getManagementFeesForUser(targetUserId, options)
